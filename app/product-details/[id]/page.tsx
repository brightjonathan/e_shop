...
export default function ProductDetailsPage() {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        // The products list now links by SKU when a product has one —
        // try that first. Falls back to a direct doc-id lookup so older
        // bookmarked/shared links (and products without a SKU) still work.
        let docId: string | null = null;
        let rawData: Record<string, unknown> | null = null;

        const skuQuery = query(collection(db, "PRODUCTS"), where("sku", "==", slug), limit(1));
        const skuSnap = await getDocs(skuQuery);

        if (!skuSnap.empty) {
          docId = skuSnap.docs[0].id;
          rawData = skuSnap.docs[0].data();
        } else {
        ...
        const raw = { id: docId, ...rawData } as Product;

        // Hidden by admin — treat the same as "not found" for customers.
  ...
  const salePrice = product && hasDiscount ? product.price - (product.price * discountPercentage) / 100 : product?.price ?? 0;
        ...
        <div className="grid gap-12 lg:grid-cols-2 lg:gap-20">
          ...
          <div className="fade-up space-y-7 pt-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {product.isNew && (
                <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold uppercase tracking-wider text-black">
                  New
                </span>
              )}
              {product.isFeatured && (
                <span className="rounded-full bg-[#fce3c7] px-3 py-1 text-xs font-semibold uppercase tracking-wider text-black">
                  ★ Featured
                </span>
              )}
              {product.category && (
            ...
            <div className="flex items-end gap-4 flex-wrap">
              <span className="text-4xl font-extrabold tracking-tight text-white">
                &#8358;{salePrice.toLocaleString("en-NG", { maximumFractionDigits: 2 })}
              </span>
              {hasDiscount && (
                <>
                  ...
                  <span className="rounded-full bg-red-500 px-2.5 py-1 text-xs font-semibold text-white mb-1">
